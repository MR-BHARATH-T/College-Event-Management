...
<div class="container-fluid">
    ...
    <form action="" id="manage-user">	
        ...
        <div class="form-group">
			...
			<option value="3" <?php echo isset($type) && $type == 3 ? 'selected' : '' ?>>Other Staff</option>
			</select>
		</div>
...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    // Get the selected department ID from the form submission
    ...
    $database = 'cwms_db'; // Database name
    ...
    // Prepare SQL statement to insert data into the user database
    $sql = "INSERT INTO users (department) VALUES (?)";
...
$sql = "SELECT id, d_name FROM department_list";
$result = $conn->query($sql);

if ($result && $result->num_rows > 0) {
    echo '<div class="form-group">';
    echo '<label for="department" class="control-label">Department</label>';
    echo '<select name="department" id="department" class="form-control form-control-sm rounded-0">';
    echo '<option value="">Select Department</option>';

    // Populate the dropdown with department names
    while ($row = $result->fetch_assoc()) {
        $department_id = $row['id']; // Store the department ID in a variable
        $department_name = htmlspecialchars($row['d_name']); // Store the department name in a variable
