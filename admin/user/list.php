				...
				<tbody>
					...
					?>
						<tr>
							...
							<td><p class="m-0 truncate-1"><?= $row['avatar'] ?></p></td>
							<td><p class="m-0 truncate-1"><?= $row['fullname'] ?></p></td>
							<td><p class="m-0 truncate-1"><?= $row['username'] ?></p></td>
							<td><p class="m-0 truncate-1"><?= $row['department'] ?></p></td>
							<?php  
							$department_name = $row['department']; // Assuming $row['department'] contains the department ID

// Get the full name of the user
$fullname = $row['fullname'];

// Prepare and execute the SQL statement to update the department_list table
$update_query = "UPDATE department_list SET HOD = ? WHERE d_name = ?";
$update_stmt = $conn->prepare($update_query);
$update_stmt->bind_param("ss", $fullname, $department_name);
$update_stmt->execute();

// Check if the update was successful
// if ($update_stmt->affected_rows > 0) {
//     echo "Full name updated for department: " . $department_name;
// } else {
//     echo "Failed to update full name for department: " . $department_name;
// }

// Close the prepared statement
$update_stmt->close();
?>
							<!-- <td><p class="m-0 truncate-1"><//?= $row['type'] == 1 ? "Admin" : "HOD" ?></p></td> -->
							<td><p class="m-0 truncate-1">
    <?= 
        $row['type'] == 1 ? "Admin" : 
        ($row['type'] == 2 ? "HOD" : "Other Staff")
    ?>
</p></td>
